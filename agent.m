function [ zadanie ] = agent( Lista_sterowan, Lista_zadan, Wielkosc_buforow)
...
%        .czasy   - czasy wykonania poszczególnych etapów zadania
    ...
    rozmiar = max(size(Lista_sterowan));
    ...
    for i=1:rozmiar
        ...
        ocena = 0;
        for j=(Lista_sterowan_krok(i).etap_zadania):liczba_etapow      % iteruj po pozostałych etapach zadaniach
            ocena = ocena +  Lista_zadan(numer_zadania).czasy(j); % zlicz czasy ich trwania
        end
        if ocena < najlepsza_ocena
            zadanie = Lista_sterowan(i);                             % Zapisz nowe najlepsze zadanie
